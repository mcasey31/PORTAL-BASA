...
export default function BasaAuthorizationsPage() {
  ...
  const { data: reviewReport = [], isLoading: isLoadingReport } = api.staff.getDependentReviewReport.useQuery(reportSearch ? { search: reportSearch } : undefined);
  ...
  const pendingAccounts = accounts.map((account) => ({
    ...account,
    dependentsAsPrincipal: account.dependentsAsPrincipal.filter((dependent) => dependent.status === "PENDING_REVIEW"),
  })).filter((account) => account.dependentsAsPrincipal.length > 0);
    ...
    <div className="space-y-7 animate-in fade-in duration-500">
      ...
      {isLoading ? <p className="py-12 text-sm text-slate-500">Cargando cuentas...</p> : error ? <p className="border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error.data?.code === "UNAUTHORIZED" ? "La sesión venció. Iniciá sesión nuevamente para consultar documentación." : error.data?.code === "FORBIDDEN" ? "Tu cuenta no tiene permisos de Staff o Administrador para consultar documentación." : "No se pudieron cargar las cuentas. Revisá la conexión e intentá nuevamente."}</p> : activeTab === "accounts" ? (
        <div className="space-y-4">
          {pendingAccounts.map((account) => (
            <article key={account.id} className="border border-slate-200 bg-white">
              ...
              {account.dependentsAsPrincipal.map((dependent) => <PendingDependentRow key={dependent.id} dependent={dependent} />)}
            </article>
          ))}
          {pendingAccounts.length === 0 && <p className="py-12 text-sm text-slate-500">No se encontraron integrantes con documentación pendiente.</p>}
        </div>
      ) : (
        ...
        <section className="border border-slate-200 bg-white">
          <div className="border-b border-slate-100 p-5"><h2 className="font-black text-slate-900">Reporte de documentación resuelta</h2><p className="mt-1 text-sm text-slate-500">Buscá aprobaciones y rechazos por DNI o nombre de titular e integrante.</p></div>
          <div className="relative max-w-xl p-5"><Search className="absolute left-9 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={reportSearch} onChange={(event) => setReportSearch(event.target.value)} placeholder="Buscar documento aprobado o rechazado" className="w-full border border-slate-200 bg-white py-3 pl-11 pr-4 text-sm font-medium outline-none focus:border-blue-500" /></div>
          <div className="overflow-x-auto"><table className="w-full min-w-175 text-left text-sm"><thead className="bg-slate-50 text-[10px] font-black uppercase tracking-widest text-slate-400"><tr><th className="px-5 py-4">Titular</th><th className="px-5 py-4">Integrante</th><th className="px-5 py-4">Resultado</th><th className="px-5 py-4">Fecha de resolución</th></tr></thead><tbody className="divide-y divide-slate-100">
            {isLoadingReport ? <tr><td colSpan={4} className="p-8 text-center text-slate-500">Cargando reporte...</td></tr> : reviewReport.map((dependent) => <tr key={dependent.id}><td className="px-5 py-4"><p className="font-bold text-slate-900">{dependent.principalPatient.user.name ?? "Titular sin nombre"}</p><p className="text-xs text-slate-500">DNI {dependent.principalPatient.dni ?? "Sin DNI"}</p></td><td className="px-5 py-4"><p className="font-bold text-slate-900">{dependent.name}</p><p className="text-xs text-slate-500">{dependent.tipoDocumentoCodigo} {dependent.dni}</p></td><td className="px-5 py-4"><StatusBadge status={dependent.status} /></td><td className="px-5 py-4 text-slate-600">{new Intl.DateTimeFormat("es-AR", { dateStyle: "short", timeStyle: "short" }).format(new Date(dependent.updatedAt))}</td></tr>)}
            {!isLoadingReport && reviewReport.length === 0 && <tr><td colSpan={4} className="p-8 text-center text-slate-500">No hay documentos aprobados o rechazados para esta búsqueda.</td></tr>}
          </tbody></table></div>
        </section>
        </div>
      )}
...
function PendingDependentRow({ dependent }: { dependent: { name: string; dni: string; tipoDocumentoCodigo: string } }) { return <div className="flex flex-col gap-3 border-b border-slate-100 p-5 last:border-b-0 md:flex-row md:items-center md:justify-between"><div className="flex items-center gap-3"><Users className="h-4 w-4 text-slate-400" /><div><p className="font-bold text-slate-900">{dependent.name}</p><p className="text-xs text-slate-500">{dependent.tipoDocumentoCodigo}: {dependent.dni}</p></div></div><StatusBadge status="PENDING_REVIEW" /></div>; }
